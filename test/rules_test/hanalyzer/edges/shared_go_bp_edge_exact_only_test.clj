(ns rules-test.hanalyzer.edges.shared-go-bp-edge-exact-only-test
  (use clojure.test
...
(deftest test-create-shared-go-edges
  (let [rule (first (filter #(= (:name %) "aael-shared-go-bp-edges-exact-only") (kabob-load-rules-from-classpath "rules/hanalyzer/edges/shared-go/aael")))
